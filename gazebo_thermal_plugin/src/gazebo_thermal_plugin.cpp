...
GazeboThermalPlugin::~GazeboThermalPlugin() { update_connection_->~Connection(); }
...
void GazeboThermalPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  double thermal_updraft =
      thermal_strength_ * std::exp(-(pos - thermal_center_).SquaredLength() / (thermal_radius_ * thermal_radius_));
